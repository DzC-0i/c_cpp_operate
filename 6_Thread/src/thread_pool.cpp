...
{
    // Create pool with 6 threads
    int task_num = 6;
    ThreadPool pool(task_num);
    ...
    std::cout << "任务数量：" << task_num << "\n"
              << "运行时长：" << std::chrono::duration_cast<std::chrono::milliseconds>(now2 - now).count() << " ms" << std::endl;
