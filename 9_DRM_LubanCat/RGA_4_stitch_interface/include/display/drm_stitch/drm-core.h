...
#define BLACK_BLUE 0X123456
    ...
    struct drm_device
    {
        uint32_t width;  // 显示器的宽的像素点数量          外部传入时需要填写
        uint32_t height; // 显示器的高的像素点数量          外部传入时需要填写
        uint32_t pitch;  // 每行占据的字节数                外部传入时需要填写
        uint32_t handle; // drm_mode_create_dumb的返回句柄
        uint32_t size;   // 显示器占据的总字节数             外部传入时需要填写
        uint32_t *vaddr; // mmap的首地址                    外部传入时需要填写
        uint32_t fb_id;  // 创建的framebuffer的id号
        int dma_fd;      // 新增：DMA缓冲区的文件描述符      外部传入时需要填写
