...
static int drm_create_fb(struct drm_device *bo)
{
    ...
    // // 使用 drmModeAddFB2 指定 FourCC 格式（更灵活）
    // uint32_t handles[4] = {bo->handle};
    // uint32_t pitches[4] = {bo->pitch};
    // uint32_t offsets[4] = {0};
    // // uint32_t fourcc = DRM_FORMAT_RGBX8888; // 或 DRM_FORMAT_XRGB8888

    // // 文档说明 DRM_FORMAT_XRGB8888 和 DRM_FORMAT_ARGB8888，一定可以正常使用，其他的不一定
