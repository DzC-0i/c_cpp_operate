...
void VideoStitch::videostitch_init()
{
...
void VideoStitch::videostitch_deinit()
{
    if (!is_videostitch_init)
    {
...
void VideoStitch::set_displaytype(DisplayType type)
{
    displaytype = type;
    is_set_resize = false;
}

void VideoStitch::set_layout(LayoutMode mode, int cell_layout_size, int rows, int cols)
{
    is_set_layout = false;
    is_set_resize = false;
    display_rects.clear();
...
void VideoStitch::calculate_automatic_layout(int cell_layout_size)
{
...
void VideoStitch::calculate_custom_layout(int cell_layout_size, int custom_rows, int custom_cols)
{
...
void VideoStitch::calculate_grid_layout(int size, int rows, int cols)
{
    // 计算单元格尺寸（考虑整除情况）
...
void VideoStitch::add_frames(size_t screen_place, cv::Mat &frame)
{
    try
    {
        if (!is_set_layout)
        ...
        try
        {
            ...
            if (src_handle_ == 0)
            {
                throw std::runtime_error("Importbuffer failed!");
            }
        }
        catch (const std::exception &e)
        {
            std::cerr << "RGA buffer creation error: " << e.what() << std::endl;
            return;
        }
        ...
        std::shared_ptr<void> src_handle_guard(nullptr, [&](void *)
                                               {
                                                try {
                                                    ...
                                                    std::cerr << "Error during buffer release (non-fatal)" << std::endl;
                                                } });
        ...
        try
        {
            ...
            {
                throw std::runtime_error(imStrError((IM_STATUS)ret));
            }
        }
        catch (const std::exception &e)
        {
            std::cerr << "RGA processing error: " << e.what() << std::endl;
            return;
        }
