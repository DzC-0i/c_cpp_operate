...
static void drm_destroy_fb(struct drm_device *bo)
{
    if (bo->dma_fd > 0)
    {
        drmModeRmFB(fd, bo->dma_fd);
        munmap(bo->vaddr, bo->size);
        close(bo->dma_fd); // 关闭DMA文件描述符
        bo->dma_fd = -1;
    }
    else
    {
...
static int drm_create_fb(struct drm_device *bo, int use_dma)
{
    int ret;

    if (use_dma && bo->dma_fd > 0)
    {
        // 使用外部DMA-BUF
        // Step 1: 将DMA-BUF转换为DRM句柄
        ret = drmPrimeFDToHandle(fd, bo->dma_fd, &bo->handle);
        if (ret < 0)
        {
            printf("drmPrimeFDToHandle failed: %s\n", strerror(errno));
            return ret;
        }

        // // Step 2: 计算对齐的步长, 外部传入
        // bo->pitch = (bo->width * 4 + 3) & ~3; // 4字节对齐
        // bo->size = bo->pitch * bo->height;

        // Step 3: 注册为FrameBuffer
        uint32_t handles[4] = {bo->handle};
        uint32_t pitches[4] = {bo->pitch};
        uint32_t offsets[4] = {0};
        ret = drmModeAddFB2(fd, bo->width, bo->height, DRM_FORMAT_XRGB8888,
...
int drm_init(struct drm_device *ext_buf)
{
    int i;
    int ret;
    // drmModeObjectProperties *props;
    ...
    // 使用外部DMA-BUF或创建新缓冲区
    if (ext_buf && ext_buf->dma_fd > 0)
    {
        buf = *ext_buf;
        ret = drm_create_fb(&buf, 1); // 1表示使用DMA模式
    }
    else
    {
        buf.width = conn->modes[0].hdisplay;
        buf.height = conn->modes[0].vdisplay;
        ret = drm_create_fb(&buf, 0); // 0表示创建DUMB
    }
    ...
    ps5.crtc_x = 0;
    ps5.crtc_y = 0;
    ps5.crtc_w = conn->modes[0].hdisplay;
    ps5.crtc_h = conn->modes[0].vdisplay;
    ps5.src_x = 0;
    ps5.src_y = 0;
    ps5.src_w = buf.width;
